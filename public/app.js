...
function updateRatioOptions() {
  ...
  if (square) {
    square.disabled = !hasImage;
  }

  if (!hasImage && ratioEl.value === "1:1") {
    ratioEl.value = "16:9";
  }
}
...
timeline.onChange = scenes => {
  console.log("Timeline:", scenes);
};

document
  .getElementById("addSceneBtn")
  .addEventListener("click", () => timeline.addScene());
...
async function generate() {
  ...
  if (!prompt && !scenes.length) {
    return message("Enter a prompt or add a scene.");
  }
...
async function poll(jobId) {
  for (;;) {
    ...
    if (!res.ok) {
      throw new Error(
        data.error || "Status request failed"
      );
    }
