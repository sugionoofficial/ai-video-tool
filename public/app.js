// AI VIDEO TOOL v1.7 - DOM READY FIX
(function () {
  function init() {
// AI VIDEO TOOL v1.6 - FREE LOCAL ROUTING FIX\nconst API_BASE = window.VIDEO_API_BASE || "";
...
async function generate() {
  ...
  try {
    const providerValue = (providerEl?.value || "").toLowerCase();
    const modelValue = (modelEl?.value || "").toLowerCase();

    // The UI may use "mock" as the option value while displaying
    // "Free Local Testing". Treat both as the free browser renderer.
    const isFreeLocal =
      providerValue.includes("free") ||
      providerValue.includes("mock") ||
      modelValue.includes("free");
