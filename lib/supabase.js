...
export async function sb(
  ...
  /*
   * Salin header caller terlebih dahulu.
   */
  const headers = {
    ...callerHeaders
  };
  ...
  /*
   * Jika request memiliki body, pastikan Supabase
   * menerima body sebagai JSON.
   *
   * Sebelumnya bagian ini tidak ada.
   * Cloudflare Fetch dapat mengirim string body
   * sebagai text/plain apabila Content-Type tidak
   * ditentukan secara eksplisit.
   *
   * PostgREST/Supabase kemudian menolak request:
   * "Content-Type not acceptable: text/plain"
   */
  if (
    safeOptions.body !== undefined &&
    safeOptions.body !== null
  ) {
    headers[
      "Content-Type"
    ] =
