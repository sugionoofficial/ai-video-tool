...
async function adminApi(request,env){const admin=await requireAdmin(request,env);const url=new URL(request.url);
  ...
  if(url.pathname==='/api/admin/transactions'&&request.method==='GET'){const userId=String(url.searchParams.get('user_id')||'').trim();const limit=Math.min(200,Math.max(1,Number(url.searchParams.get('limit')||100)));if(!userId)throw new HttpError('user_id wajib.',400);const tx=await sb(`/rest/v1/credit_transactions?user_id=eq.${encodeURIComponent(userId)}&select=id,amount,balance_after,type,note,job_id,admin_user_id,created_at&order=created_at.desc&limit=${limit}`,{},env);if(!tx.ok)throw new HttpError('Gagal mengambil riwayat credit.',500);return json({success:true,transactions:await tx.json()},200,env)}
